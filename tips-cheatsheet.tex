...
\usepackage{enumitem}       % For controlling lists (like enumerate start number)
\usepackage{fancyhdr}       % For custom headers and footers
\usepackage{hyperref}       % For PDF metadata and potential links
\usepackage{lipsum}         % For placeholder text (not used in final doc, but good for testing)

% --- PAGE GEOMETRY ---
% Adjust margins for a cheatsheet feel (more text, less margin)
\geometry{
    a4paper,
    left=2cm,
    right=2cm,
...
\lhead{The Pragmatic Programmer - 100 Tips Cheatsheet} % Left header
\rhead{Page \thepage}        % Right header with page number
\renewcommand{\headrulewidth}{0.4pt} % Line below header
\renewcommand{\footrulewidth}{0pt}   % No line above footer

% --- PDF METADATA ---
\hypersetup{
    pdftitle={The Pragmatic Programmer - 100 Tips Cheatsheet},
    pdfauthor={zucchivan},
    pdfsubject={Summary of the 100 tips from The Pragmatic Programmer, 20th Anniversary Edition},
    pdfkeywords={pragmatic programmer, programming, software development, tips, summary, cheatsheet},
    colorlinks=false,       % Set true for colored links if needed
...
\begin{center}
    ...
    \vspace{2cm}
    {\Huge\bfseries Tip Cheatsheet\par} % Changed line
    \vspace{3cm}

    % --- Abstract --- Start
    \begin{minipage}{0.8\textwidth} % Constrain width of abstract
        \large % Slightly larger font for abstract
        \hrule % Rule above abstract
        \vspace{0.5em} % Space after rule
        ...
        \vspace{0.5em} % Space before rule
        \hrule % Rule below abstract
    \end{minipage}
    % --- Abstract --- End

    \vfill % Pushes content to top/bottom
    \rule{\textwidth}{0.4pt} % Horizontal line
    \ Sourced from the book by David Thomas and Andrew Hunt
\end{center}
\clearpage % Start content on the next page

% --- MAIN CONTENT ---
\pagestyle{fancy} % Apply the fancy page style from here onwards

\section*{Chapter 1: A Pragmatic Philosophy}
\begin{enumerate}[label=\arabic*., itemsep=1ex] % Start list, add some space between items
...
\newpage % Ensure next chapter starts on a new page (optional, LaTeX might do this anyway)
